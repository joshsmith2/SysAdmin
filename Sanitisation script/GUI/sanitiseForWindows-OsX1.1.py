...
try:
		opts, args = getopt.getopt(sys.argv[1:], "o:l:r:e:dhqbc", ["oversizelog=","log=", "root=", "errorlog=", "dorename","help","quiet","debug","casesensitive"])
...
if opts == []:
   ...
   options = {}
   options['parent'] = root
   options['title'] = "Directory to Search"
   theRoot = askdirectory(**options)
   quiet = True
   options['title'] = "Save report here"
   logFileName = asksaveasfilename(**options)
