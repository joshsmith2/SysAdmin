...
#Either renames a file, or logs it. 
	...
	if rename:
		try:
					...
					if not quiet:
						print "Changed from: " + prevPath
						print "Changed to:   " + newPath + "\n"
					if logFileName is not None:
						logFile.write( "Changed from: " + prevPath + "\n")
						logFile.write( "Changed to:	" + newPath + "\n")
		except OSError:
					print "Error: unable to rename " + prevPath
					if logFileName is not None:
						logFile.write("Error: unable to rename " + prevPath)
		...
		if not quiet:
			print "Would change:    " + prevPath
			print "Would change to: " + newPath + "\n"
	
		if logFileName is not None:
			logFile.write( "Would change:	 " + prevPath + "\n")
			logFile.write( "Would change to: " + newPath + "\n")
...
try:
		opts, args = getopt.getopt(sys.argv[1:], "o:l:r:dhqbc", ["oversizelog=","log=", "root=", "dorename","help","quiet","debug","casesensitive"])
